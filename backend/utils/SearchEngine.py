...
class CodeSearchEngine:
    def __init__(self, chroma_path=None, collection_name="code_chunks", embedding_model=None, chunks_filepath=None):
    ...
    def llm_search(self, query: str, k: int = 10) -> Dict[str, Dict]:
        ...
        try:
            basic_llm_func = getattr(self.embedding_model, 'basic_llm', None)
            if callable(basic_llm_func):
                enriched_query = basic_llm_func(query)
            else:
        ...
        combined = {}
        # Handle case where results might be empty or not structured as expected
        if results and "metadatas" in results and results["metadatas"] is not None and len(results["metadatas"]) > 0:
            metadatas = results["metadatas"][0]
            distances_list = results.get("distances")
            if distances_list is not None and len(distances_list) > 0:
                distances = distances_list[0]
...
if __name__ == "__main__":
    ...
    while True:
        ...
        for i, res in enumerate(results, 1):
            meta = res["metadata"]
            line_info = f"{meta.get('start_line', '?')}  {meta.get('end_line', '?')}"
